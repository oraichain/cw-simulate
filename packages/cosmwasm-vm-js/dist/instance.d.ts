...
export declare const GAS_COST_HUMANIZE = 44;
export declare const GAS_COST_CANONICALIZE = 55;
export declare const GAS_COST_LAST_ITERATION = 37;
export declare const GAS_COST_RANGE = 11;
...
export declare class VMInstance {
    ...
    debugMsgs: string[];
    static eddsa: eddsa;
    private static wasmMeteringCache;
    private static wasmCache;
    constructor(backend: IBackend, env?: Environment);
    build(wasmByteCode: Uint8Array, checksum?: string): Promise<void>;
    ...
    curve_hash(input_ptr: number, curve: number, destination_ptr: number): number;
    poseidon_hash(left_input_ptr: number, right_input_ptr: number, curve: number, destination_ptr: number): number;
    groth16_verify(input_ptr: number, public_ptr: number, vk_ptr: number, curve: number): number;
    ...
    do_poseidon_hash(left_input: Region, right_input: Region, curve: number, destination: Region): Region;
    do_groth16_verify(input: Region, proof: Region, vk: Region, curve: number): number;
    do_debug(message: Region): void;
    do_query_chain(request: Region): Region;
    do_abort(message: Region): void;
    instantiate(env: Env, info: MessageInfo, msg: object): object;
    execute(env: Env, info: MessageInfo, msg: object): object;
    query(env: Env, msg: object): object;
    migrate(env: Env, msg: object): object;
    sudo(env: Env, msg: object): object;
    reply(env: Env, msg: object): object;
    ibc_channel_open(env: Env, msg: object): object;
    ibc_channel_connect(env: Env, msg: object): object;
    ibc_channel_close(env: Env, msg: object): object;
    ibc_packet_receive(env: Env, msg: object): object;
    ibc_packet_ack(env: Env, msg: object): object;
    ibc_packet_timeout(env: Env, msg: object): object;
}
